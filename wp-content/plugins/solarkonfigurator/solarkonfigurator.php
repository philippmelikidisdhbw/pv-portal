...
function solarkonfigurator_shortcode() {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if(isset($_POST['requestOffer'])){
            ...
            // -------------------------------------------
            // E-MAIL VERSENDEN MIT WP MAIL SMTP
            // -------------------------------------------
            ...
            // HEADERS SO ANPASSEN, DASS WP MAIL SMTP GREIFT
            // WICHTIG: KEIN "From:" überschreiben, WP Mail SMTP regelt das!
            $headers = array(
                'Content-Type: text/html; charset=UTF-8',
                'Reply-To: ' . ($data['vornameNachname'] ?: 'Kunde') . ' <' . ($data['email'] ?: 'noreply@domain.de') . '>'
            );
        ...
        if(isset($_POST['sendEmail']) && $_POST['sendEmail'] == "An E-Mail senden") {
            ...
            // -------------------------------------------
            // E-MAIL VERSENDEN (BERICHT AN KUNDEN) MIT WP MAIL SMTP
            // -------------------------------------------
            $to = $data['email'];
            $subject = "Ihr unverbindlicher Bericht";
            // Keine From-Zeile setzen (WP Mail SMTP/Gmail übernimmt das). Nur Reply-To, HTML-Format.
            $headers = array(
                'Content-Type: text/html; charset=UTF-8',
                'Reply-To: Solar Solutions <noreply@solarsolutionsgmbh.com>'
            );
