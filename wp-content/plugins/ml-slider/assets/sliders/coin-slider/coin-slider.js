...
(function ($) {
	...
	$.fn.coinslider = $.fn.CoinSlider = function (options) {
		...
		var updateActiveClass = function (el) {
			var slides = $('#' + el.id + ' img');
		
			slides.removeClass('active prev next');
		
			slides.each(function (index) {
				if (index === imagePos[el.id]) {
					$(this).addClass('active');
				} else if (index === (imagePos[el.id] - 1 + slides.length) % slides.length) {
					$(this).addClass('prev');
				} else if (index === (imagePos[el.id] + 1) % slides.length) {
					$(this).addClass('next');
				}
			});
		};
		...
		var transition = function (el, direction) {
			...
			updateActiveClass(el); // Neue Funktion aufrufen
		};
